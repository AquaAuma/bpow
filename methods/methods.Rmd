---
title: "Appendix 1. Methods file describing all data processing steps to create the GIS layer."
author: "A. Maureaud, G. Reygondeau, K. Ingenloff, J.G. Vigneron, L. Watling, K. Winner, W. Jetz"
date: '2023'
output:
  pdf_document: default
  word_document: default
  html_document: 
    toc: true
    theme: flatly
bibliography: bpow_refs.bib
link-citations: yes
---

```{r setup, include=FALSE, results='hide'}
knitr::opts_chunk$set(echo = TRUE)
library(here)
```

*Brief description:* This document presents the combination of steps performed either on R v4.0.3 (R Core Team 2021), or on the geospatial software (ArcPro v2.8.3, but reproducible on the open source QGIS) to create the biogeographic layer of the Benthic Provinces of the World (BPOW).

# Data

-   MEOW: Marine Ecoregions of the World: 232 ecoregions, 62 provinces, and 12 realms [Spalding et al. 2007](https://doi.org/10.1641/B570707) [@spalding_marine_2007]
-   DSP: Deep sea GOODS provinces: 14 bathyal and 14 abyssal provinces [Watling et al. 2013](https://doi.org/10.1016/j.pocean.2012.11.003) [@watling_proposed_2013]
-   Hadal provinces [Belyaev, 1989](https://escholarship.org/uc/item/46n6148x) [@belyaev_deep_1989]
-   Global depth raster from [GEBCO](https://www.gebco.net/) [@gebco_bathymetric_compilation_group_gebco_2020_2020]
-   Land polygons with major islands from [Natural Earth](https://www.naturalearthdata.com/downloads/10m-physical-vectors/10m-land/) at 10m resolution, version 4.1.0

# Analysis steps

## Part I: Geometry checks (DSP layer) on ArcPro

*Performed in ArcPro v2.8.3*

-   Step #1: We checked both the bathyal and abyssal layers for errors as we noticed invalid geometries when trying to perform geometric operations in ArcPro/QGIS. Specifically, numerous points were flagged as having self-intersections.

    [![Examples of self-intersections in the shapefiles visualized on QGIS](images/self_intersection_1.png "Examples of self-intersections in the shapefiles visualized on QGIS")](figure1) ![](images/self_intersection_2.png) ![](images/self_intersection_3.png "Examples of self-intersections in the shapefiles visualized on QGIS")

-   Step #2: We evaluated the validity of geometries using "Check validity" in QGIS `Vector → Geometry Tools → Check Validity → Input Layer: BATHYAL/ABYSSAL`

    ![](images/self_intersection_4.png)

## Part II: Geometry checks (DSP layer) on R

*Performed in R v4.0.3*

-   Step #1: We used the function `sf::st_is_valid()` to checked the validity of geometries in each layer. All issues were due to self-intersections in both objects.

-   Step #2: We used the function `sf::st_make_valid()` to correct identified self-intersections in each file.

-   Step #3: Files were saved using the function `sf::st_write()`.

```{r code a., code = readLines(here("./code/create_layer/a.fix_deep_sea_layers.R")), class.source="small", eval = FALSE}
```

## Part III: Fix boundary

*Performed in R v4.0.3*

-   Step #1: We identified areas off the coast of Florida, USA with spatial irregularities resulting from the fact that the 800m threshold is arbitrary and this is a "transition" zone.

    ![](images/irregularities.png)

-   Step #2: We addressed the irregularites in R using GEBCO and adding additional areas to the bathyal layer.

-   Step #3: All polygons intersecting the depth raster (in blue below) were merged using `sf::st_union()` to create a single larger replacing polygons, added to the bathyal layer.

    ![](images/irregularities_3.png)

-   Step #4: We again checked geometry validity to ensure no errors (e.g., self-intersections) remained and saved the file using `sf::st_write()`.

```{r code b., code = readLines(here("./code/create_layer/b.remove_irregularities.R")), class.source="small", eval = FALSE}
```

## Part IV: Creating complementary bathyal and abyssal layers

*Performed in ArcPro v2.8.3*

-   Step #1: We assessed whether or not the bathyal and abyssal layers are perfectly complementary to each other by manual visualization. They were not.

    ![](images/complementarity_1.png) ![](images/complementarity_2.png) ![](images/complementarity_3.png)

-   Step #2: To create perfectly complementary abyssal and bathyal layers we took the difference between the two layers. All grid cells that are shared between the abyssal and bathyal habitats were associated with the bathyal layer because they may represent more potential habitat than abyssal areas.

    Operation: geometric difference between the bathyal layer and the abyssal layer

    `ArcGIS: Geoprocessing → Pairwise Erase, input = abyssal, erase = bathyal, output = p4s2`

-   Step #3: We created a single, merged shapefile (the deepsea layer) that included the newly complementary abyssal and bathyal layers.

    Operation: merging of the two complementary shapefiles into a single deep sea shapefile

    `ArcGIS: Merge, input = p4s2, input = GOODSprovinces_bathyal_irregularities, output = GOODSprovinces_p4s3`

-   Step #4: Manual correction of invalid geometries

    ![](images/self_intersection_5.png)

## Part V: Deep-sea clip

*Performed in ArcPro v2.8.3*

-   Step #1: Here, two options are possible: (i) removing marine ecoregion areas and keeping them associated with the deep-sea layers (ii) removing areas from the deep sea areas and keeping them associated with the ecoregions. Land areas are identified with the natural earth file because the coastal ecoregions already include more than coastlines and don't need to include the best coastline resolution.

    ![](images/merge_1.png)

    Operation (i): geometric difference between the MEOW layer and the DEEPSEA layer

    ![](images/merge_2.png)

    `ArcGIS: Geoprocessing → Pairwise Erase, input = meows_ecos, erase = GOODSprovinces_p4s3, output = provinces_p5s1`

-   Step #2: Creating a shapefile for joining the layers MEOW and DEEPSEA that after Step #1 should be perfectly complementary (no spatial overlap) that I will then call the SEAFLOOR shapefile layer.

    `ArcGIS: Geoprocessing → Merge, input = provinces_p5s1, input = GOODSprovinces_p4s3, output = provinces_p5s2`

-   Step #3: extract provinces_p5s2 as a shapefile

-   Step #4: fix the format of provinces_p5s2 in R

```{r code c., code = readLines(here("./code/create_layer/c.layer_post-processing_1.R")), class.source="small", eval = FALSE}
```

## Part VI: Indentification of uncharacterized sites

*Performed in ArcPro v2.8.3*

-   Step #1: two types of areas will not yet be characterized in the SEAFLOOR layer, (i) hadal regions below 6,500m, where we should not really observe species, (ii) regions between 200 and 800m that don't belong to ecoregions. The bathyal layer starts at 800m and onwards, so there are remaining zones to associate with a region.

    ![](images/missing_regions_1.png) ![](images/missing_regions_2.png)

-   Step #2: get a shapefile with all missing polygons: (merge of seafloor shapefile with a low resolution land layer (does not matter since meows have a bunch of land areas already)

    `-ArcGIS: Pairwise erase, input = ne_10m_land, erase = Provinces_P5S2, output = Provinces_P5S2_PairwiseErase`

    `-ArcGIS: Merge, input = Provinces_P5S2_PairwiseErase, input = Provinces_P5S2, output = Provinces_P5S2_merge`

-   Step #3: get polygons that are uncharacterized

    `-ArcGIS: Create feature, extent of -180;180;-90,90`

    `-ArcGIS: Pairwise erase, input = extent, erase = Provinces_P5S2_PairwiseErase, output = holes_p6s2`

## Part VII: Isolation and classification of hadal trenches

*Performed in R v4.0.3*

-   Step #1: Identification of hadal trenches of the world are done based on rough identification of relevant coastal ecoregions based on [@belyaev_deep_1989] and [@jamieson2010].

-   Step #2: For each identified ecoregion from Step #1, we use the corresponding depth raster from GEBCO to identify trenches (below 6,500m deep) and create new polygons from there. For instance, on the figure below, the ecoregion "Eastern Caribbean" from provinces_p5s2 include deep area on the northern sites.

    ![](images/select_hadal_1.png) ![](images/select_hadal_2.png)

-   Step #3: All polygons are merged back to the provinces shapefile, where coastal ecoregions do not include hadal trenches anymore, and they are added as separate geometries.

```{r code d., code = readLines(here("./code/create_layer/d.remove_hadal_from_meows.R")), class.source="small", eval = FALSE}
```

## Part VIII: Assigning biogeographic regions to unclassified polygons

*Performed in R v4.0.3*

-   Step #1: associate the depth raster corresponding to a box around the polygon

-   Step #2: find the polygons of the seafloor in that box

-   Step #3: if there is only one polygon, associate the missing polygon the that polygon

-   Step #4: if there are multiple polygons, calculate the closest distance based on 3D (longitude, latitude, depth (a)) with the function [mcNNindex](https://www.rdocumentation.org/packages/Morpho/versions/2.8/topics/mcNNindex) and associate the closest polygon based on shortest distance around each point of the raster of the missing polygon (b). Because some areas are transition areas, the boundaries between the polygons are not clean (see figure (c)), so the resulting raster is aggregated at a lower resolution, as well as the grid cell polygon associations (d). Finally, polygons are created and associated with the ID of the closest polygon from the seafloor shapefile.

    ![](images/3D-NNA_1.png) ![](images/3D-NNA_2.png) ![](images/3D-NNA_3.png)

```{r  code e., code = readLines(here("./code/create_layer/e.add_missing_regions.R")), class.source="small", eval = FALSE}
```

-   Step #5: finalize the shapefile

```{r code f., code = readLines(here("./code/create_layer/f.full_areas.R")), class.source="small", eval = FALSE}
```

## Part IX: Quality checks

*Performed in ArcPro v2.8.3*

-   Step #1: Remove unnecessary parts of the geometry

    `-ArcGIS: Pariwise Dissolve, input = bpow_s8p5, by=ID`

```{r code h., code = readLines(here("./code/create_layer/h.fix_geometry_prior_clipp.R")), class.source="small", eval = FALSE}
```

-   Step #2: Fix the boundaries of the layer

    `-ArcGIS: Creation of boundary box to clip the layer`

## Part X: Clipping to global landmasses

*Performed in ArcPro v2.8.3*

`-ArcGIS: Pairwise Erase, input = land, erase = bpow_s9`

```{r code i., code = readLines(here("./code/create_layer/i.add_attributes.R")), class.source="small", eval = FALSE}
```

# References
